...
> 本文保留 v4 基线定义。当前实现继续读取 v4，并通过 [v5 删除及 WebP 扩展](edit-package-v5.md) 和 [v6 图片引用扩展](edit-package-v6.md) 增加能力；不含删除或图片引用的编辑仍可导出 v4。
...
历史页面将两种编号分开：内置资料的“第 n 次更新”按历史记录追加顺序从 1 编号，展示时倒序排列（最新在前）；`revision` 仅显示为“编辑包修订版 rN”。浏览器更新包列表的序号表示应用顺序，移动更新包只改变应用顺序，不改变修订号。不修改历史数据或更新包结构。
