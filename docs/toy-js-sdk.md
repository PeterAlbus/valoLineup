...
SDK 不能替代当前项目的 Lineup 内容存储和编辑机制：云存储按“登录用户 + Toy”隔离，容量为每个 Toy 最多 128 个键值对，单个 value 不超过 1024 字节；它不适合作为所有访问者共享的 Lineup 数据源。当前 `content/*.yaml` 仍应作为地图、英雄和 Lineup 内容的权威来源，本地编辑接口仍只服务于开发阶段的内容维护。
...
不适合存储：完整地图数据、全部 Lineup 记录、图片、共享内容元数据或本地编辑器的权威保存结果。
...
当前 Lineup 数据使用可选 `videoUrl` 保存任意 HTTP/HTTPS 教学视频链接。SDK 的 `navigate` 不接收任意 URL，而是接收受支持的 B站资源类型和资源 ID。

因此两者不能无条件互换：

- 当链接指向 B站视频且能够获得 BV 号时，可以在支持 `navigate` 的环境使用原生视频跳转。
- 当链接是其他视频网站或普通网页时，仍需使用标准 `<a>` 导航。
- 数据模型目前只保证 URL 合法，不保证能解析出 BV 号；接入前必须明确 BV 号来自结构化字段还是可靠解析。
...
项目的编辑模式通过本地开发服务把修改写回 `content/lineups.yaml`，并将图片写入仓库的 `public/lineups`。静态 Toy 页面没有本地文件写权限，Toy 云存储也没有共享内容管理、图片对象存储或仓库写回能力。

因此 SDK 不会让当前本地编辑接口自动变成可在线部署的编辑后台。未来若要求在 Toy 页面中维护所有用户共享的 Lineup，需要独立设计服务端内容存储与鉴权；这不属于 SDK 1.7.0 提供的能力。
...
3. 对 B站教学视频接入 `navigate`，同时保留现有通用 URL 的标准跳转语义。
