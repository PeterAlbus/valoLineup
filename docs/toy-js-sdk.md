...
项目不依赖写入接口。编辑模式把坐标、BV 号、新点位和待上传图片保存在当前浏览器页面中；用户点击“导出编辑包”后下载一个 ZIP。开发者在仓库运行 `pnpm run content:import-edits -- /path/to/package.zip`，CLI 会逐条合并新增内容，并用每条已有 Lineup 的修改前快照检测并发冲突。冲突 Lineup 会被跳过，其他内容继续导入。
