...
SDK 不能替代当前项目的 Lineup 内容存储和编辑机制：云存储按“登录用户 + Toy”隔离，容量为每个 Toy 最多 128 个键值对，单个 value 不超过 1024 字节；它不适合作为所有访问者共享的 Lineup 数据源。`content/*.yaml` 是公共点位来源；浏览器使用 localStorage 存包清单和持续编辑层，IndexedDB 存图片。v4 ZIP 支持浏览器导入或由开发者永久导入仓库，详见 [更新包协议](edit-package-v4.md)。
...
项目不依赖服务端写入接口。点击编辑先通过 `getUserProfile()` 获取昵称和可选 `toyOpenId`，SDK 不提供 UID，因此新上传者不存 UID，也不提供主页跳转。已有 PeterAlbus / 2003822 资料保留，点击通过 `navigate({type:'space',id:'2003822'})` 跳转。

保存编辑将累计变更存到 localStorage，图片写入 IndexedDB。导出下载同一个持续编辑包，不清空草稿。开发者运行 `pnpm run content:import-edits -- /path/to/package.zip`，CLI 根据修改前快照检测冲突并记录实际更新历史；浏览器历史页导入则按包顺序无条件覆盖相同 ID，最后叠加手动编辑。
...
分享、二维码、容器控制、云存储、排行榜、作者关系和媒体采集不属于页面现有行为；用户资料已用于点位上传者和编辑包作者归属。
