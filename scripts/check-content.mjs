...
assert.equal(content.lineups.length, 47, 'Preserve all imported lineups');
...
assert.equal(content.lineups.filter((lineup) => lineup.videoBvid).length, 23, 'Preserve the imported teaching videos');
...
assert.equal(groupIds.size, 43, 'Preserve all imported destinations');
...
assert.equal(media.length, 90, 'Preserve all imported images');
...
console.log('Content checks passed: 47 lineups, 43 destinations, 90 WebP media files.');
