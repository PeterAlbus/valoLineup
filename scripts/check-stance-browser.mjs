...
try {
  ...
  assert(!await evaluate(`Boolean(document.querySelector('${marker(first.id)}'))`));
  ...
  await click(`.method-list button[data-lineup-id="${second.id}"]`);await click('.media-section .stance-add');await place(.7,.6);
  assert.equal(await evaluate("document.querySelectorAll('.stance-pin').length"),3, 'Both editing and reading show the nearby methods, including the path endpoint');
  await click(marker(first.id));assert.equal(await evaluate("document.querySelector('[aria-label=点位名称]').value"),first.id);
  assert(await evaluate(`document.querySelector('.method-list button[data-lineup-id="${first.id}"]').getAttribute('aria-pressed')==='true'`));
  ...
  await button('编辑路径');await button('重置');assert(!await evaluate(`Boolean(document.querySelector('${marker(pathRecord.id)}'))`),'Reset path preview removes only its derived stance');
  assert.equal(await evaluate("document.querySelectorAll('.stance-pin').length"),2,'Other nearby methods keep their stance markers');
  await click('.geometry-cancel');await wait(`document.querySelector('${marker(pathRecord.id)}')`);
